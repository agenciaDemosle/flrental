...
const csvFile = fs.readFileSync('./docs/Datos de equipos FL Rental.xlsx - Semi-Remolques .csv', 'utf-8');
const lines = csvFile.split('\n');

console.log('Iniciando importación de semi-remolques...\n');
...
async function importProducts() {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const capacidadMax = values[4];
    const ejes = values[5];
    const largoUtil = values[6];
    const notas = values[7];
    const cantidad = parseInt(values[8]) || 1;
    ...
    if (capacidadMax && capacidadMax !== 'N/A') {
      description += `<p><strong>Capacidad Máxima:</strong> ${capacidadMax}</p>`;
    }
    if (ejes && ejes !== 'N/A') {
      description += `<p><strong>Ejes:</strong> ${ejes}</p>`;
    }
    if (largoUtil && largoUtil !== 'N/A') {
      description += `<p><strong>Largo Útil:</strong> ${largoUtil}</p>`;
    }
    if (notas && notas !== 'N/A') {
    ...
    const productData = {
      ...
      meta_data: [
        ...
        { key: 'capacidad_max', value: capacidadMax },
        { key: 'ejes', value: ejes },
        { key: 'largo_util', value: largoUtil },
  ...
  console.log('\n=== Resumen de Importación de Semi-Remolques ===');
